...
import { Search, MapPin, Calendar, Trash2, Ticket, ArrowRight, LayoutDashboard } from 'lucide-react';
...
const UserDashboard = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Failed to find registrations. Please check the email and try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-5xl mx-auto space-y-12 py-10">
      ...
      >
        ...
        <p className="text-slate-500 dark:text-slate-400 text-xl max-w-2xl mx-auto font-medium">
          Enter the email address you used to register to view your upcoming experiences.
        </p>
      </motion.div>
      ...
      <motion.div 
        ...
        className="bg-white dark:bg-[#111827] p-3 rounded-full shadow-2xl shadow-indigo-500/5 dark:shadow-none border border-slate-200 dark:border-white/5 max-w-2xl mx-auto"
      >
        <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-center w-full gap-2">
          <div className="hidden sm:flex pl-5 pr-2">
          ...
          <input
            ...
            required
            className="flex-1 w-full bg-transparent border-none focus:ring-0 text-slate-900 dark:text-white placeholder-slate-400 py-4 px-6 sm:px-0 outline-none text-lg font-medium"
          />
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className="w-full sm:w-auto bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-8 py-4 rounded-full font-bold transition-colors disabled:opacity-70 whitespace-nowrap shadow-lg flex items-center justify-center gap-2"
          >
            {loading ? (
        ...
        >
          {registrations.length === 0 ? (
            <div className="text-center py-20 bg-white dark:bg-[#111827] rounded-[3rem] border border-slate-200 dark:border-white/5 shadow-xl">
              ...
              <p className="mt-2 text-slate-500 font-medium text-lg mb-8">We couldn't find any registrations under "{email}".</p>
              <Link to="/">
                <motion.button 
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
